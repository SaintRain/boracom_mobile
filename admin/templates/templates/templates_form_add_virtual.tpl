...
<form id="data form" action="?act=templates&do=insert_virtual&tamplate_id={$tamplate_id}" method="POST" onsubmit="return Mysubmit(this)" style="margin:0px">
  ...
  <table class="formborder" border="0" width="100%" cellpadding="1" cellspacing="0">
    <tr>
      ...
      <table width="100%" class="formbackground" border="0" cellpadding="2" cellspacing="2">
          <tr>
            <td colspan="2">{$MSGTEXT.tpl_name} <font color="yellow">*</font><br>
          ...
          <tr>
            <td nowrap><b>{$MSGTEXT.tag_caption}</b></td>
            <td nowrap><b>{$MSGTEXT.you_cat_edit_tags_caption}</b></td>
          </tr>
          {foreach from=$tags item=item}
          <tr>
            <td nowrap>{$item.tagname} </td>
            <td width="100%"><input type="text" {if ($item.global==1) } style="color:blue;width:100%" {else} {if ($item.global==2) } style="color:maroon;width:100%" {else} style="width:100%"{/if} {/if} name="tagid{$item.id}" value="{$item.tagname}"></td>
          </tr>
          {/foreach}
          {/if}
          <tr>
            <td></td>
            <td><input class="button" type="submit" value="{$MSGTEXT.create}" style="width:130px" ></td>
          </tr>
        </table>
        </td>
    </tr>
