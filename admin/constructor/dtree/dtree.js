...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		...
		str += '<a TARGET=\"basefrm\" id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="index.php' + node.url + '"';
...
dTree.prototype.getSelected = function() {
	flag=false;
 	...
 	if (urlParts[1]) {
		variables=urlParts[1].split("&");	
		if  (variables[0]) {
			tmpArray=variables[0].split("=");	
