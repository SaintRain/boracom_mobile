...
Форма контактов
*////////////////////////////////////////////////////////////////////////////////////////////
class ShowForm extends Contacts {
	...
	function send() {
		...
		if($this->settings['kcaptcha']==1) {
			if(isset($_SESSION['captcha_keystring2']) && $_SESSION['captcha_keystring2'] == $this->post['kcaptcha']){
			}
			else{
				$this->errors[]	= 'Неверно введён текст с изображения.';
			}

		}
           
      			//если поле должно быть обязательно заполнено
			if (!$this->post['UserEmail'] && !$this->post['UserPhone']) {
				$this->errors[]	=  sprintf('Укажите телефон или email!');
			}
