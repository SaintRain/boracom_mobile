...
{/literal}
<form id="data form" action="?act=administrators&do=saveedit&id={$id}" method="POST" onsubmit="return Mysubmit(this)" style="margin:0">
  ...
  <table class="formborder" border="0" cellpadding="1" cellspacing="0">
	...
	<td>      
      <table class="formbackground" border="0" cellpadding="2" cellspacing="2">
          <tr>
            <td>{$MSGTEXT.login}: </td>
            <td><input type="text" style="width:300px; background-color:#DBDBDB" readonly name="login" value="{$login}"></td>
          </tr>
          <tr>
            <td>{$MSGTEXT.email}: <font color="Yellow">*</font></td>
            <td><input type="text" style="width:300px" name="email" value="{$email}"></td>
          </tr>
          <tr>
          ...
          <tr>
            <td>            
            <a href="index.php?act=administrators&do=group_edit">{$MSGTEXT.group}</a>: <font color="Yellow">*</font></td>
            <td><select name="group_id" style="width:300px">
                ...
                <option {if $group.id==$group_id} selected {/if} value="{$group.id}">{$group.name}</option>                
				{/foreach}
              </select></td>
          </tr>
          <tr>
