...
*////////////////////////////////////////////////////////////////////////////////////////////
class Registration extends InternetShop {
	...
	function check_reg () {
		...
		if (!isset($this->post['confirm'])) {
			$this->errors[]	= 'Вы должны подтвердить достоверность указанных данных!';
		}
		...
		if ($this->posts['nic']!='') {
			...
			$this->errors[]	= 'Вы не указали ник на форуме!';
		}
		...
		if (count($this->errors)>0) {
			...
			$this->dataSynchronize($this->tablePrefix.'users', $this->settings['forum_users_table_name'], $api->inserted_id);

			//делаем отправку
			$mail	= $FRAME_FUNCTIONS->getMailObject($this->posts['email'], $this->posts['name'], SETTINGS_EMAIL_USERNAME, SETTINGS_EMAIL_CAPTION, $this->settings['send_com_theme_confirm'], $body);
			$res	= $mail->send();
	...
	function confirm_r() {
		...
		if ($is_exists && $is_exists==$this->gets['id']) {
			...
			$this->dataSynchronize($this->tablePrefix.'users', $this->settings['forum_users_table_name'], $is_exists);
