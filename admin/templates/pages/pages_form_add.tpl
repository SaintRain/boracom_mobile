...
function Mysubmit(form) {
	s=form.name.value;
	if (s=='') {
		form.name.focus(); alert("{/literal}{$MSGTEXT.pages_add_mess_err_name}{literal}"); return false
	};
	return true;
}
  ...
  <table class="formborder" border="0" width="100%" cellpadding="1" cellspacing="0">
	...
	<td>  
      <table class="formbackground" width="100%" border="0" cellpadding="2" cellspacing="2">
        <tr>
          <td valign="top">{$MSGTEXT.pages_add_description}</td>
              ...
              <tr>
                ...
                <td nowrap><table border="0" cellpadding="0" cellspacing="0" >
                    <tr>
                      ...
                      <td>&nbsp;{$MSGTEXT.pages_add_translit_mess}</td>
                    </tr>
                  </table>
                  </td>
              </tr>
        ...
        <tr>
          <td width="100">{$MSGTEXT.pages_add_name_page} <font color="yellow">*</font></td>
          <td><input type="text" style="width:100%" name="name" value="{$name}" id="transVal"></td>
        </tr>
        <tr>
          <td>{$MSGTEXT.pages_add_tamplate} <font color="yellow">*</font></td>
          <td><select name="templates_id" style="width:100%">
              <option style="color:gray" value="0" >{$MSGTEXT.pages_add_no_tamplate}
        ...
        <tr>
          <td>{$MSGTEXT.pages_add_category} </td>
          </td>        
          <td><select name="page_category" id="page_category" style="width:100%" >
              <option value="0" style="color:gray">{$MSGTEXT.pages_add_no_parent}
              {if $pageCategories}
        ...
        <tr>
          <td></td>
          <td><input class="button" type="submit" value="{$MSGTEXT.pages_add_create}" style="width:120px;margin-right:5px">
            &nbsp;&nbsp;
            <input class="button" type="button" onclick="javascript:window.history.back()" value="{$MSGTEXT.pages_add_cancel}" style="width:120px"></td>
        </tr>
      </table>
        </td>
    </tr>
