...
function getTemplate(tpl_name, c_number) {
	...
	xmlObject2.open("GET", "ajax.php?func=getTemplate&tpl_name="+tpl_name+"&c_number="+c_number+"&f_id="+f_id+"&sourse_field_id="+sf_id+"&delete="+delete_value+"&own_filter="+own_filter_value+"&module_id={$smarty.session.___GoodCMS.m_id}&table_id="+t_id+"&selected_edit_type="+selected_edit_type+"&time="+time ,false);
	{literal}
...
function getTablesGet() {
	if (xmlObject2.readyState == 4) {
		...
		el=new Option({/literal}"{$MSGTEXT.editData_no_table}"{literal} ,0);
		el.style.color='gray';
		...
		el=new Option({/literal}"{$MSGTEXT.editData_no_esteblishid}"{literal} ,0);
...
function getFieldsGet() {
	if (xmlObject2.readyState == 4) {
		...
		el=new Option({/literal}"{$MSGTEXT.tables_edit_not_set}"{literal} ,0);
		el.style.color='gray';
		objSel.options[objSel.options.length] =el;
...
function createFieldsFromList() {
	...
	selectByValue('edittype_id'+counter, '{$field.edittype_id}');
	GetElementById('sourse_field_id'+counter).value='{$field.sourse_field_id}';
	GetElementById('delete'+counter).value='{$field.delete}';
	GetElementById('own_filter'+counter).value='{$field.own_filter}';
	

	img=GetElementById('imgsettings'+counter);
	edittype=GetElementById('edittype_id'+counter).value;
	if ((edittype==3 || edittype==4 || edittype==6 || edittype==14 || edittype==15) && ({$field.datatype_id}!=24 && {$field.datatype_id}!=25)) img.style.visibility='visible';
	else img.style.visibility='hidden';

	GetElementById('sort_index'+counter).value='{$field.sort_index}';
	GetElementById('field_id'+counter).value='{$field.id}';
	GetElementById('comment'+counter).value='{$field.comment}';
	GetElementById('fieldname'+counter).value='{$field.fieldname}';
	selectByValue('datatype_id'+counter, '{$field.datatype_id}');
	GetElementById('len'+counter).value='{$field.len}';
	GetElementById('default'+counter).value='{$field.default}';
	//selectByValue('collation_id'+counter, '{$field.collation_id}');
	
	GetElementById('group_caption'+counter).value='{$field.group_caption}';
	
	GetElementById('pk'+counter).checked={if $field.pk==1}true{else}false{/if};
	GetElementById('not_null'+counter).checked={if $field.not_null==1}true{else}false{/if};
	GetElementById('unsigned'+counter).checked={if $field.unsigned==1}true{else}false{/if};
	GetElementById('auto_incr'+counter).checked={if $field.auto_incr==1}true{else}false{/if};
	GetElementById('zerofill'+counter).checked={if $field.zerofill==1}true{else}false{/if};
	GetElementById('unique'+counter).checked={if $field.unique==1}true{else}false{/if};
...
function beforeSubmit() {
	...
	if (form.elements.length==8) {
		alert("{/literal}{$MSGTEXT.tables_edit_alert_one}{literal}");
		return false
	}
...
function collationsList() {
	{/literal}
	collation_text='<select disabled onfocus="setcolor(\'newRow'+counter+'\');" name="collation_id'+counter+'" id="collation_id'+counter+'" style="width:100%" >{foreach from=$collations item=collation}<option value="{$collation.id}" >{$collation.collation}{/foreach}</select>';
...
function datatypesList() {
	{/literal}
	datatype_text='<select disabled onChange="setType('+counter+')" onfocus="setcolor(\'newRow'+counter+'\');" name="datatype_id'+counter+'" id="datatype_id'+counter+'" style="width:100%" >{foreach from=$datatypes item=datatype}<option value="{$datatype.id}" >{$datatype.datatype}{/foreach}</select>';
...
function edittypeList() {
	...
	edittype_text='<table border=0 cellpadding=0 cellspacing=0><tr><td><input type="hidden" name="delete'+counter+'" id="delete'+counter+'" value="0"><input type="hidden" name="own_filter'+counter+'" id="own_filter'+counter+'" value="0"><input type="hidden" name="sourse_field_id'+counter+'" id="sourse_field_id'+counter+'" value="" ><img hspace=0 id="imgsettings'+counter+'" alt="{$MSGTEXT.tables_edit_sourse}" style="cursor:pointer;visibility:hidden" onClick="setDataMap('+counter+')" src="images/f_s.gif" align="left" border=0></td><td width="2">&nbsp;<td><td><select disabled onChange="showHideImg('+counter+')" onfocus="setcolor(\'newRow'+counter+'\');" name="edittype_id'+counter+'" id="edittype_id'+counter+'" style="width:125" ><option value="0" selected style="color:gray">Not Edit{foreach from=$edittypes item=edittype}<option value="{$edittype.id}" >{$edittype.edittype}{/foreach}</select></td></tr></table>';
...
function getTemplateByWindow(tpl_name, c_number) {
	...
	f_id=GetElementById('field_id'+c_number).value;
	url="ajax.php?func=getTemplate&tpl_name="+tpl_name+"&f_id="+f_id+"&module_id={$smarty.session.___GoodCMS.m_id}&time="+time+'&t_id={$id}';
...
function checkIncrenent(c_number) {
	...
	if (edittype_id==4 && (datatype_id!=2 && datatype_id!=5  && datatype_id!=6 && datatype_id!=7 && datatype_id!=8)) {
		alert("{/literal}{$MSGTEXT.tables_edit_alert_text}{literal}");
...
function q(){
	return confirm("{/literal}{$MSGTEXT.tables_edit_alert_del}{literal}");
}
...
<form id="editForm" action="?act=t_c&do={$do}{if $id}&t_id={$id}{/if}" method="POST" style="margin:0px" onsubmit="return beforeSubmit()">
      ...
      <table align="center" cellpadding="1" cellspacing="0" bgcolor="#74a9d3" border="0"  style="width:100%">
          <tr>
            <td><table width="100%" class="formbackground" border="0" cellpadding="2" cellspacing="2">
                <tr>
                  <td> {$MSGTEXT.tables_edit_name_table} <font color="Yellow">*</font><br>
                ...
                <tr>
                  <td> {$MSGTEXT.tables_edit_description}<font color="Yellow">*</font><br>
                    <input type="text" name="description" style="width:100%;" value="{$description}"></td>
                </tr>
                <tr>
                  <td> {$MSGTEXT.tables_edit_additional_buttons}<br>
                    <textarea type="text" name="additional_buttons" style="width:100%;height:30px">{$additional_buttons}</textarea></td>
                </tr>                
                <tr>
                  <td><table cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td><input name="show_type" type="checkbox" class="checkbox" value="1" {if $show_type==1} checked {else}{if !$name} checked {/if}{/if}></td>
                ...
                <tr>
                  <td><p style="margin-top:0px; margin-bottom:2px"><b>{$MSGTEXT.tables_edit_fields_table}</p>
                    <table width="100%" id='tab1' cellpadding="2" cellspacing="1" border="0" bgcolor="White">
                      <tbody>
                        <tr style="height:19px">
                          ...
                          <td class="top_list" valign="top" width="10%">{$MSGTEXT.tables_edit_edited}</td>
                          <td class="top_list" valign="top" width="14%" nowrap>{$MSGTEXT.tables_edit_desc_field}</td>
                          <td class="top_list" valign="top" nowrap width="12%">{$MSGTEXT.tables_edit_name_field}</td>
                          <td class="top_list" valign="top" width="11%" nowrap>{$MSGTEXT.tables_edit_type_data}</td>
                          <td class="top_list" valign="top" width="5%" nowrap>{$MSGTEXT.tables_edit_long}</td>
                          <td class="top_list" valign="top" width="6%">Default</td>
                          <!--
                          <td class="top_list" valign="top" width="14%" nowrap>{$MSGTEXT.tables_edit_coding}</td>
                          -->
                          <td class="top_list" valign="top" width="5%" nowrap>{$MSGTEXT.tables_edit_group}</td>
                ...
                <tr>
                  ...
                  <table cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td><input style="width:170px" value="{$MSGTEXT.tables_edit_del_field}" id='deleteButton' disabled class="button" onclick="deleteField()" type="button"></td>
                        <td width="25px"></td>
                        <td><input style="width:130px" value="{$MSGTEXT.tables_edit_add_field}" class="button" onclick="addNewField()" type="button"></td>
                        <td width="5px"></td>
                        <td><input style="width:130px" value="{$MSGTEXT.tables_edit_save}" class="button" type="submit" type="button"></td>
                      </tr>
                    </table>
                    </td>
                </tr>
