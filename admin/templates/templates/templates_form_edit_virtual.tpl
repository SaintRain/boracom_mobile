...
<form id="data form" action="?act=templates&do=saveedit_virtual&id={$id}{if $smarty.get.hide_menu}&hide_menu=true{/if}" method="POST" onsubmit="return Mysubmit(this)" style="margin:0px">
    ...
    <tr>
      ...
      <table width="100%" class="formbackground" border="0" cellpadding="2" cellspacing="2">
          ...
          <tr>
            ...
            <td width="100%"><input type="text" {if $item.global==1} style="color:blue;width:100%" {else} {if $item.global==2} style="color:maroon;width:100%" {else} style="width:100%"{/if} {/if} name="virtualtagid{$item.virtualtag_id}" value="{$item.virtualtagname}"></td>
          </tr>
          {/foreach}
          {/if}
          <tr>
            <td></td>
            <td><input class="button" type="submit" value="{$MSGTEXT.tag_names_save}" style="width:130px"></td>
          </tr>
        </table>
        </td>
    </tr>
