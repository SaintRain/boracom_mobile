...
Meta - описание
*////////////////////////////////////////////////////////////////////////////////////////////
class Metadescription extends InternetShop {
	...
	function START() {
		...
		if (isset($this->gets['id'])) {
			$query 					= "SELECT t.metadescription FROM `{$this->tablePrefix}products` AS `t` WHERE t.id='{$this->gets['id']}'";
			$table_name				= $this->tablePrefix.'products';
		}
		elseif (isset($this->gets['category_id'])) {
			$query 					= "SELECT t.metadescription FROM `{$this->tablePrefix}categories` AS `t` WHERE t.id='{$this->gets['category_id']}'";
